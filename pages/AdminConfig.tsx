...
export const AdminConfig = () => {
  const [activeTab, setActiveTab] = useState<'advisors' | 'indicators' | 'budgets' | 'users' | 'history' | 'profile'>('advisors');
...
const BudgetCard: React.FC<BudgetCardProps> = ({ indicator, targetId, periodType, budgets, year, week, onSave, forcedValue, isLocked }) => {
      ...
      if (isLocked && forcedValue !== null && forcedValue !== undefined) {
          ...
          if (needsSave) {
              const timer = setTimeout(() => {
                   if (periodType === 'WEEKLY') { onSave([{ id: crypto.randomUUID(), indicatorId: indicator.id, targetId, periodType: 'WEEKLY', year, week, amount: forcedValue }], year, week); } 
                   else {
                      const configs: any[] = [];
                      days.forEach(d => { configs.push({ id: crypto.randomUUID(), indicatorId: indicator.id, targetId, periodType: 'DAILY', dayOfWeek: d.val, year, week, amount: forcedValue }); });
    ...
    const handleWeeklyChange = (val: string) => {
      if (isLocked) return;
      onSave([{ id: crypto.randomUUID(), indicatorId: indicator.id, targetId, periodType: 'WEEKLY', year, week, amount: parseFloat(val) || 0 }], year, week);
    };
    ...
    const handleDailyChange = (dayIndex: number, val: string) => {
      ...
      Object.entries(newVals).forEach(([d, v]) => {
        ...
        configs.push({ id: crypto.randomUUID(), indicatorId: indicator.id, targetId, periodType: 'DAILY', dayOfWeek: parseInt(d), year, week, amount: numVal });
      });
      
      setWeeklyVal(calculatedWeekly.toString());
      configs.push({ id: crypto.randomUUID(), indicatorId: indicator.id, targetId, periodType: 'WEEKLY', year, week, amount: calculatedWeekly });
