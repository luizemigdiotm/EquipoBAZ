...
import { Trophy, Filter, X, ChevronLeft, Users, Briefcase, BarChart2, Ghost, Printer, Share2, Award, Medal, CheckCircle, AlertTriangle, Target, MessageCircle, Clock } from 'lucide-react';
import html2canvas from 'html2canvas';

export const Dashboard = () => {
  const { records, advisors, indicators, budgets } = useData();
  ...
  const calculateSpecificTarget = (
    ...
    if (fType === 'WEEK') {
        ...
        if (weeklyConfig) {
            ...
            totalTarget = weekBudgets.filter(b => b.periodType === 'DAILY').reduce((sum, b) => sum + b.amount, 0);
        }
    } else if (fType === 'DAY') {
        const weekBudgets = relevantBudgets.filter(b => b.year === year && b.week === week);
        ...
        if (dailyConfig) {
            totalTarget = dailyConfig.amount;
             ...
             const weeklyConfig = weekBudgets.find(b => b.periodType === 'WEEKLY');
             if (weeklyConfig) {
                 if (isAverage) totalTarget = weeklyConfig.amount; // Don't divide % by 7
  ...
  const calculateAdjustedDailyCommitment = (
      ...
      if (isDeficit) {
          // Math.ceil ensuring integer
          return { value: Math.ceil(todayBaseTarget + Math.abs(difference)), isDeficit: true };
      } else {
...
const IndicatorHistoryModal = ({ indicator, year, week, quarter, filterType, records, budgets, viewMode, selectedAdvisorId, advisors, indicators, onClose }: any) => {
    ...
    const historyData = useMemo(() => {
        ...
        return weeks.map(w => {
             ...
             return { name: `Sem ${w}`, week: w, actual: Math.ceil(actual), target: Math.ceil(target) };
    ...
    const tableData = useMemo(() => {
        ...
        if (filterType === 'WEEK') {
            ...
            const weeklyConfig = relevantBudgets.find((b: any) => b.periodType === 'WEEKLY');
            const weeklyTargetTotal = weeklyConfig ? weeklyConfig.amount : relevantBudgets.reduce((s:number, b:any) => s+b.amount, 0);
            
            // Fix: If isAverage, don't divide by 7
            const dailyTargetFallback = isAverage ? weeklyTargetTotal : weeklyTargetTotal / 7;

            rows = days.map(day => {
                const dailyB = relevantBudgets.find((b: any) => b.periodType === 'DAILY' && b.dayOfWeek === day.id);
                const target = Math.ceil(dailyB ? dailyB.amount : dailyTargetFallback);
                const dailyR = currentRecords.find((r: RecordData) => r.frequency === 'DAILY' && r.dayOfWeek === day.id);
                const real = Math.ceil(dailyR ? (dailyR.values[indicator.id] || 0) : 0);
                const dailyPrev = prevRecords.find((r: RecordData) => r.frequency === 'DAILY' && r.dayOfWeek === day.id);
                const prev = Math.ceil(dailyPrev ? (dailyPrev.values[indicator.id] || 0) : 0);
                totalReal += real; totalTarget += target; totalPrev += prev;
                return { label: day.label, target, real, prev };
            });
            const weeklyRec = currentRecords.find((r: RecordData) => r.frequency === 'WEEKLY'); if (weeklyRec) totalReal = Math.ceil(weeklyRec.values[indicator.id] || 0);
            const weeklyPrevRec = prevRecords.find((r: RecordData) => r.frequency === 'WEEKLY'); if (weeklyPrevRec) totalPrev = Math.ceil(weeklyPrevRec.values[indicator.id] || 0);
        } else {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4 print:fixed print:inset-0 print:bg-white print:z-50 print:p-0 print:h-screen print:w-screen">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto flex flex-col print:shadow-none print:max-w-none print:max-h-none print:w-full print:h-full print:rounded-none">
                ...
                <div ref={modalRef} className="p-6 space-y-8 print:p-8 bg-white">
                     ...
                     <div className="h-[300px] border rounded-lg p-2 bg-white print:border-none"><h4 className="text-sm font-bold text-gray-700 mb-2">Historial Anual</h4><ResponsiveContainer width="100%" height="100%"><BarChart data={historyData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="name" angle={-45} textAnchor="end" height={60} interval={0} fontSize={10}/><YAxis /><ReTooltip formatter={(value: number) => indicator.unit === '$' ? `$${value.toLocaleString()}` : value}/><Legend /><Bar dataKey="actual" name="Real" fill="#0ea5e9" radius={[4, 4, 0, 0]} /><Bar dataKey="target" name="Meta" fill="#cbd5e1" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div>
                     <div className="border rounded-lg overflow-hidden"><div className="bg-gray-100 p-3 font-bold text-gray-700 border-b flex justify-between print:bg-gray-50"><span>Desglose {filterType === 'WEEK' ? 'Diario' : 'Semanal'}</span><span className="text-xs font-normal text-gray-500 mt-1">Comparativo vs Meta y Anterior</span></div><div className="overflow-x-auto"><table className="w-full text-sm text-right"><thead className="bg-gray-50 text-xs text-gray-500 uppercase print:bg-white print:border-b"><tr><th className="px-4 py-2 text-left">{filterType === 'WEEK' ? 'Día' : 'Semana'}</th><th className="px-4 py-2 bg-yellow-50 print:bg-white">{filterType === 'WEEK' ? 'Sem. Ant' : 'Per. Ant'}</th><th className="px-4 py-2">Objetivo</th><th className="px-4 py-2 font-bold bg-blue-50 text-blue-900 print:bg-white print:text-black">Real</th><th className="px-4 py-2">Vs Objetivo</th><th className="px-4 py-2">Vs Anterior</th></tr></thead><tbody className="divide-y">{tableData.rows.map((row: any) => (<tr key={row.label} className="hover:bg-gray-50"><td className="px-4 py-2 text-left font-medium">{row.label}</td><td className="px-4 py-2 bg-yellow-50 text-gray-600 print:bg-white">{row.prev.toLocaleString(undefined, {maximumFractionDigits:0})}</td><td className="px-4 py-2">{row.target.toLocaleString(undefined, {maximumFractionDigits:0})}</td><td className="px-4 py-2 font-bold bg-blue-50 text-blue-800 print:bg-white print:text-black">{row.real.toLocaleString(undefined, {maximumFractionDigits:0})}</td><td className={`px-4 py-2 ${row.diff >= 0 ? 'text-green-600' : 'text-red-600'}`}><div className="flex flex-col text-xs"><span>{row.diff > 0 ? '+' : ''}{row.diff.toLocaleString(undefined, {maximumFractionDigits:0})}</span><span>({(row.diffPct * 100).toFixed(0)}%)</span></div></td><td className={`px-4 py-2 ${row.prevDiff >= 0 ? 'text-green-600' : 'text-red-600'}`}><div className="flex flex-col text-xs"><span>{row.prevDiff > 0 ? '+' : ''}{row.prevDiff.toLocaleString(undefined, {maximumFractionDigits:0})}</span><span>({(row.prevPct * 100).toFixed(0)}%)</span></div></td></tr>))}<tr className="bg-gray-100 font-bold border-t-2 border-gray-300 print:bg-gray-50"><td className="px-4 py-2 text-left">TOTAL {isAverage ? '(Promedio)' : ''}</td><td className="px-4 py-2">{tableData.totals.prev.toLocaleString(undefined, {maximumFractionDigits:0})}</td><td className="px-4 py-2">{tableData.totals.target.toLocaleString(undefined, {maximumFractionDigits:0})}</td><td className="px-4 py-2 text-blue-900 text-lg print:text-black">{tableData.totals.real.toLocaleString(undefined, {maximumFractionDigits:0})}</td><td className="px-4 py-2 text-center text-gray-500">-</td><td className="px-4 py-2 text-center text-gray-500">-</td></tr></tbody></table></div></div>
