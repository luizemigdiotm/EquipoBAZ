...
export const DataProvider = ({ children }: { children?: ReactNode }) => {
    ...
    const saveBudget = async (configs: BudgetConfig[], year: number, week: number) => {
        ...
        // Batch 1: Inserts (No ID key in payload)
        if (inserts.length > 0) {
            const dbInserts = inserts.map(mapKeysToDB);
            // Request return=representation to get the generated IDs
        ...
        try {
            ...
            // Collect all saved items from server response
            const allSavedDB: any[] = [];
            for (const res of responses) {
                const json = await res.json();
                if (Array.isArray(json)) {
                    allSavedDB.push(...json);
                }
            }

            const allSavedApp = mapKeysToApp(allSavedDB) as BudgetConfig[];
            ...
            // CRITICAL: Do NOT call fetchData() here. 
            // It causes a full reload which might be slower than this local update
            // and can cause UI flickering or "ghosting" if DB replication is lagging.
            // fetchData(); 

        } catch (error) {
            console.error('[saveBudget] Error saving/updating:', error);
            // Only if error, maybe try to fetch to restore truth?
            // fetchData(); 
            throw error;
        }
    };
